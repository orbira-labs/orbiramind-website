...
const EXTENDED_FILTERS: FilterOption[] = [
  { id: "pending_analysis", label: "Analiz Bekleyen", shortLabel: "Bekleyen", icon: FileText, color: "text-pro-warning" },
  { id: "report_ready", label: "Raporu Hazır", shortLabel: "Hazır", icon: FileText, color: "text-pro-success" },
  { id: "upcoming_appointment", label: "Randevusu Yaklaşan", shortLabel: "Randevu", icon: Calendar, color: "text-pro-accent" },
  { id: "no_contact_7d", label: "7+ Gün Temas Yok", shortLabel: "7+ gün", icon: AlertCircle, color: "text-pro-danger" },
];
...
export function ClientFilters({
  ...
  return (
    <>
      {/* ============================================================
          DESKTOP VIEW - Yatay filter bar (değiştirilmedi)
          ============================================================ */}
      <div className="desktop-only space-y-3">
      ...
      {/* ============================================================
          MOBILE VIEW - Yatay scroll chips
          ============================================================ */}
      <div className="mobile-only space-y-2">
        {/* Status filter chips - horizontal scroll */}
        <div className="mobile-scroll-snap gap-2 px-4">
          <button
            onClick={() => onStatusChange("all")}
            className={clsx(
              "mobile-chip touch-manipulation",
              statusFilter === "all" && "mobile-chip-active"
            )}
          >
            Tümü
          </button>
          {CLIENT_STATUSES.map((s) => (
            <button
              ...
              className={clsx(
                "mobile-chip touch-manipulation",
                statusFilter === s.id && "mobile-chip-active"
              )}
            >
              {s.label}
            </button>
          ...
          {onToggleExtended && (
            <button
              ...
              className={clsx(
                "mobile-chip touch-manipulation",
                (showExtended || hasActiveExtended) && "mobile-chip-active"
              )}
            >
              <Filter className="h-3.5 w-3.5" />
              {hasActiveExtended ? `(${extendedFilters.length})` : "Filtre"}
            </button>
          )}
        </div>

        {/* Extended filters - horizontal scroll chips */}
        {showExtended && (
          <div className="mobile-scroll-snap gap-2 px-4 pb-2">
            {EXTENDED_FILTERS.map((filter) => {
              const Icon = filter.icon || Filter;
              const isActive = extendedFilters.includes(filter.id);
            ...
            {hasActiveExtended && (
              <button
                onClick={() => extendedFilters.forEach(f => onExtendedFilterToggle(f))}
                className="mobile-chip text-pro-danger touch-manipulation"
              >
                Temizle
              </button>
            )}
