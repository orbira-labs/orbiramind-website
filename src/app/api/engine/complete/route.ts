...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { session_id, deep_dive_answers } = body;
    ...
    const response = await fetch(`${ENGINE_API_URL}/v1/sessions/${session_id}/complete`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ deep_dive_answers }),
    });
    ...
    try {
      ...
      console.error("Engine complete: yanıt JSON parse edilemedi, status:", response.status);
      return NextResponse.json(
    ...
    console.error("Engine complete API error:", err);
    return NextResponse.json(
